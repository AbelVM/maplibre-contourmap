    ...
    const
        ...
        shoot = e => {
            if (e.sourceId === source && e.isSourceLoaded){
                ...
                const 
                    sourcefeatures = e.target.querySourceFeatures(source, query_options),
                    dedup = sourcefeatures.filter((value, index, self) =>
                        index === self.findIndex(t => (
                            t.properties[o.measure] === value.properties[o.measure] && 
                            t.geometry.coordinates[0] === value.geometry.coordinates[0] && 
                            t.geometry.coordinates[1] === value.geometry.coordinates[1]
                        ))
                    ),
                    features = dedup.map((f, i) => {
                        const g = f.geometry;
                        return {
                            "type": "Feature",
                            "id": i,
